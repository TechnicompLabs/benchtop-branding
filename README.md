...
- **tc-benchtop-branding** - the default desktop background: `usr/share/backgrounds/tc-benchtop/`, its entry in Settings, Appearance (`usr/share/gnome-background-properties/`), and `usr/share/glib-2.0/schemas/90-tc-benchtop-branding.gschema.override`, which makes it the default. The override sorts after openSUSE's `20_openSUSE-branding.gschema.override`, and glib-compile-schemas lets a later file replace an earlier file's value.
...
The source path is made of straight segments, so the logo is visibly faceted when drawn very large. That does not show at the sizes generated here (64 px and below for the light logos, 128 px for the icons). The wallpaper is a separate PNG and does not come from the SVG.
...
The artwork is CC-BY-SA-4.0 (`COPYING.artwork`); the spec, `tools/make-logos` and the configuration files are MIT (`LICENSE`).
